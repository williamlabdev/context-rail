...
// Store persists one document baseline per Project.
type Store interface {
	Load(projectID string) (*State, error)
	Save(projectID string, state *State) error
}

// FileStore keeps documents.json per Project under dir with atomic writes.
...
func (store *FileStore) path(projectID string) (string, error) {
	...
	return filepath.Join(store.dir, projectID, "documents.json"), nil
}
...
func (store *FileStore) Load(projectID string) (*State, error) {
	...
	if err != nil {
		return nil, newError(CodeStateUnavailable, "read document baseline: %v", err)
	}
	var state State
	if err := json.Unmarshal(raw, &state); err != nil {
		return nil, newError(CodeStateUnavailable, "decode document baseline: %v", err)
	}
...
func (store *FileStore) Save(projectID string, state *State) error {
	...
	if err != nil {
		return newError(CodeStateUnavailable, "encode document baseline: %v", err)
	}
	tmp, err := os.CreateTemp(filepath.Dir(path), "documents-*.json.tmp")
	if err != nil {
