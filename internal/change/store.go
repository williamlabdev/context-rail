...
// FileStore keeps one changes.json ledger per Project under dir, written
// atomically (temp file + rename) so the ledger is never half-written.
...
func (store *FileStore) path(projectID string) (string, error) {
	...
	return filepath.Join(store.dir, projectID, "changes.json"), nil
}
...
func (store *FileStore) Load(projectID string) (*State, error) {
	...
	if err != nil {
		return nil, newError(CodeStateUnavailable, "read change ledger: %v", err)
	}
	var state State
	if err := json.Unmarshal(raw, &state); err != nil {
		return nil, newError(CodeStateUnavailable, "decode change ledger: %v", err)
	}
...
func (store *FileStore) Save(projectID string, state *State) error {
	...
	if err != nil {
		return newError(CodeStateUnavailable, "encode change ledger: %v", err)
	}
	tmp, err := os.CreateTemp(filepath.Dir(path), "changes-*.json.tmp")
	if err != nil {
